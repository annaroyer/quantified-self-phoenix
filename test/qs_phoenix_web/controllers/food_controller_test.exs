defmodule QsPhoenixWeb.FoodControllerTest do
  ...
  describe "show food" do
    test "renders food with given id", %{conn: conn} do
      conn = get conn, food_path(conn, :show, 1)
      assert json_response(conn, 200) == %{"id" => 1,
      "name" => "Banana",
      "calories" => 150}
    end

    test "renders food with different given id", %{conn: conn} do
      conn = get conn, food_path(conn, :show, 4)
      assert json_response(conn, 200) == %{"id" => 4,
      "calories" => 550,
      "name" => "Yogurt"}
    end

    test "returns 404 when food with given id does not exist", %{conn: conn} do
      assert_error_sent :not_found, fn ->
         get conn, food_path(conn, :show, 9)
      end
    end
  end
  ...
  describe "create food" do
    test "renders food when data is valid", %{conn: conn} do
      ...
      assert json_response(conn, 201) == %{"id" => 8, "name" => "Crackers", "calories" => 200}
    end
    ...
    test "renders errors when there are no calories", %{conn: conn} do
      conn = post conn, food_path(conn, :create), food: @no_calories
      assert response(conn, 400) == ""
    end
  end
